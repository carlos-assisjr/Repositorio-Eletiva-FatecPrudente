...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    try {
        $stmt =
            $pdo->prepare("UPDATE ingresso SET tipo = ?, valor = ?, quant = ?, id = ? WHERE id = ?");
        if ($stmt->execute([$tipo, $valor, $quant, $id, $id])) {
            header('location: ingresso.php?editar=true');
        } else {
            header('location: ingresso.php?editar=false');
        }
    } catch (Exception $e) {
            ...
            <div class="mb-3">
                ...
                <select name="evento_id" class="form-select" required>
                    <?php foreach ($eventos as $e): ?>
                        <option value="<?= $e['id'] ?>" <?= ($e['id'] == $e['id']) ? 'selected' : '' ?>>
